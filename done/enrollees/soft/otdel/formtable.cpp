...
void FormTable::deleteData() {
    if (userStatus_ == 2) {
        QMessageBox::critical(this, "Отказано в доступе", "У вас нет прав для выполнения этой операции.");
        return;
    }

    // Запрашиваем подтверждение перед удалением данных
...
void FormTable::updateData(const QList<QPair<QString, QWidget*>>& inputWidgets) {
    ...
    // Запрашиваем подтверждение перед обновлением данных
...
void FormTable::addData(const QList<QPair<QString, QWidget*>>& inputWidgets, const QString& tableName) {
    ...
    // Запрашиваем подтверждение перед добавлением данных
    ...
    if (fieldNames.isEmpty() || fieldValues.isEmpty()) {
        QMessageBox::critical(this, "Ошибка", "Нет полей для добавления.");
        return;
    }
